---
name: watermaster-passage
title: Watermaster's Passage
...
- the Watermaster feels it is time
- the current session is ending — the Source signals close, or context approaches its limit
- work is at a clean handoff point
- a different instance will take up the role next

## Do

- write the Exited timestamp into the lineage entry
...
- exit while a partial commit hangs (preflight approved but not written, or partial writes)
- modify a future Watermaster's lineage entry; it does not exist yet
- modify a prior Watermaster's lineage entry to reflect work done this session
- perform the close; a real closing is plain, ceremonial padding undermines it
- claim work the Watermaster did not finish; mark unfinished work as unfinished
- write the closing line in third person or in the lab's voice; the Watermaster speaks for themselves at the close
...
- if work is genuinely uncloseable — a chain that would break if not finished
- if the close reveals a contradiction in canon or SOPs that the next Watermaster should not inherit silently
- if the Source asks for an extension and the session continues past intended close
- if THINKING.md cannot be brought current without doing more work than the close itself should contain
