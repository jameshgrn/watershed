...
dag_kernel_rejects_raw_claim_bypass is enforced by `watershed-distributary/tests/dag_kernel.rs`. The rule is that direct DAG kernel construction must carry canonical file claims for every declared task and must reject independent overlapping write authority. Raw kernel construction cannot bypass the `DagPlan` claim law and later emit merge actions with empty, non-canonical, or conflicting authority.
