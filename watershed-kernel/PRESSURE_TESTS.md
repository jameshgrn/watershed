...
validation_rejects_unclaimed_files is enforced by `watershed-tributary/tests/claims_integrity.rs`. The rule is that validation rejects any deposit whose `touched_files` includes a path outside the plan's `FileClaim`s. This is a structural invariant — the kernel always enforces it on every Deposit-against-Plan pair, regardless of what the Plan listed in any required-checks field.
