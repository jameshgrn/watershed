...
fn retry_lineage_changes_run_id() {
    ...
    let original = derive_run_id(plan.intent(), plan.claims(), None, 0);
    let retry = derive_run_id(plan.intent(), plan.claims(), Some(original.as_str()), 1);
...
fn equivalent_retry_lineage_produces_equal_run_ids() {
    let plan = validated_plan("derive run id", "a.rs", ClaimKind::Exclusive);
    let retry_a = derive_run_id(plan.intent(), plan.claims(), Some("run:parent"), 1);
    let retry_b = derive_run_id(plan.intent(), plan.claims(), Some("run:parent"), 1);
...
fn differing_retry_index_produces_different_run_ids() {
    let plan = validated_plan("derive run id", "a.rs", ClaimKind::Exclusive);
    let retry_a = derive_run_id(plan.intent(), plan.claims(), Some("run:parent"), 1);
    let retry_b = derive_run_id(plan.intent(), plan.claims(), Some("run:parent"), 2);
